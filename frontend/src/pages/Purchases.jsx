...
import { Home, LogOut, BookOpen, BadgeIndianRupee, ShoppingBag, Clock, AlertCircle } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchPurchases = async () => {
      try {
        ...
        // Use same format as your courses response
          ...
          <div className="p-8">
            <nav className="space-y-4">
              ...
              <button 
                ...
                className="w-full flex items-center gap-4 px-6 py-4 rounded-xl bg-white/5 border border-purple-800/50 hover:bg-red-900/30 hover:border-red-600 transition-all group"
              >
              ...
              <div className="max-w-md mx-auto">
                <div className="bg-red-900/20 border border-red-800/30 rounded-2xl p-12">
                  ...
                  <Button 
                    ...
                    className="bg-linear-to-r from-purple-600 to-fuchsia-600 hover:from-purple-500 hover:to-fuchsia-500 text-white font-semibold rounded-xl px-8 py-3"
                  >
                    Try Again
                  </Button>
                </div>
          ...
          {!loading && !error && purchases.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {purchases.map((purchase) => {
                // Get course data - adjust based on your backend response
                const course = purchase.courseId || purchase;
                  ...
                  >
                    <Link to={`/course/${course._id}`} className="block">
                      <div className="bg-white/5 backdrop-blur-xl border border-purple-800/40 rounded-2xl overflow-hidden shadow-2xl hover:border-purple-600/70 transition-all duration-300">
                        {/* Course Image - SAME AS COURSES.JSX */}
                        ...
                        {/* Course Details - SAME ATTRIBUTES AS COURSES.JSX */}
                        <div className="p-6 space-y-4">
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-purple-800/30">
                            {/* Price - SAME AS COURSES.JSX */}
                            <span className="text-3xl font-bold bg-linear-to-r from-purple-400 to-fuchsia-400 bg-clip-text text-transparent">
                              ${course.price}
                            </span>

                            {/* View Course Button */}
                            <Button className="bg-linear-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white font-semibold rounded-xl px-6 shadow-lg">
                              View Course
                            </Button>
                          </div>
                        </div>
